...
class RAGFlowPdfParser:
    def __init__(self):
        ...
        try:
            ...
            json_path = os.path.join(model_dir, "updown_concat_xgb.json")
            bin_path = os.path.join(model_dir, "updown_concat_xgb.model")
            if os.path.exists(json_path):
                self.updown_cnt_mdl.load_model(json_path)
            else:
                self.updown_cnt_mdl.load_model(bin_path)
        except Exception:
    ...
    def _images(self, fnm, zoomin=3, page_from=0,
                page_to=299, callback=None):
        return self.__images__(fnm, zoomin, page_from, page_to, callback)
