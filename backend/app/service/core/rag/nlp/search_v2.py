...
def _cosine_scores(query_vector, vectors) -> np.ndarray:
    ...
    denominator = np.linalg.norm(query_vector) * np.linalg.norm(vectors, axis=1)
    return np.divide(
        vectors @ query_vector,
        denominator,
        out=np.zeros(vectors.shape[0], dtype=float),
        where=denominator != 0,
    )
...
class Dealer:
    ...
    def get_vector(self, txt, emb_mdl, topk=10, similarity=0.1):
        ...
        if qv is None:
            raise RuntimeError("Failed to generate query embedding for RAG retrieval")
    ...
    def rerank(self, sres, query, tkweight=0.3,
        ...
        ins_tw = []
        for i in sres.ids:
            content_ltks = _split_tokens(sres.field[i].get(cfield, ""))
            title_tks = _split_tokens(sres.field[i].get("title_tks", ""))
            question_tks = _split_tokens(sres.field[i].get("question_tks", ""))
    ...
    def rerank_fast(self, sres):
        ...
        vtsim = _cosine_scores(sres.query_vector, vectors)
        order_bonus = np.linspace(1e-6, 0, num=len(vtsim), dtype=float)
        sim = vtsim + order_bonus
        tsim = np.zeros(len(vtsim), dtype=float)
        return sim, tsim, vtsim
    ...
    def retrieval(self, question, embd_mdl, tenant_ids, kb_ids, page, page_size, similarity_threshold=0.2,
        ...
        RERANK_PAGE_LIMIT = 3
        candidate_limit = _env_int("RAG_RERANK_CANDIDATES", 30)
        candidate_limit = max(page_size * RERANK_PAGE_LIMIT, min(candidate_limit, top))
        req = {"kb_ids": kb_ids, "doc_ids": doc_ids, "size": candidate_limit,
               "question": question, "vector": True, "topk": candidate_limit,
        ...
        logging.info(
            "RAG search returned total=%s candidates=%s in %.2fs",
            sres.total,
            len(sres.ids),
        ...
        if page <= RERANK_PAGE_LIMIT:
            ...
            rerank_mode = "remote" if (sres.total > 0 and rerank_mdl is not None) else "vector"
            if sres.total > 0 and rerank_mdl is not None:
                logging.debug("Reranking retrieval results with model")
                sim, tsim, vsim = self.rerank_by_model(rerank_mdl,
                                                       sres, question, 1 - vector_similarity_weight,
                                                       vector_similarity_weight,
                                                       rank_feature=rank_feature)
            elif sres.total > 0:
                sim, tsim, vsim = self.rerank_fast(sres)
            else:
                sim, tsim, vsim = self.rerank(
                    sres, question, 1 - vector_similarity_weight, vector_similarity_weight,
                    rank_feature=rank_feature)
            logging.info(
                "RAG %s rerank processed %s candidates in %.2fs",
                rerank_mode,
                len(sres.ids),
