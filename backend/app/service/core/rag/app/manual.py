...
from service.core.api.db import ParserType
from io import BytesIO
from service.core.rag.nlp import rag_tokenizer, tokenize, tokenize_table, bullets_category, title_frequency, tokenize_chunks, docx_question_level, naive_merge
from service.core.rag.utils import num_tokens_from_string
from service.core.deepdoc.parser import PdfParser, PlainParser, DocxParser
...
def _simple_tokenize(text, include_unigrams=False):
...
def _fast_tokenize_chunks(chunks, doc):
    res = []
    for ck in chunks:
        if len(ck.strip()) == 0:
            continue
        ...
        d["content_ltks"] = _simple_tokenize(ck)
        d["content_sm_ltks"] = _simple_tokenize(ck, include_unigrams=True)
        res.append(d)
    return res
...
def chunk(filename, binary=None, from_page=0, to_page=100000,
    ...
    pdf_parser = None
    plain_pdf_mode = bool(
        re.search(r"\.pdf$", filename, re.IGNORECASE)
        and kwargs.get("layout_recognize", "DeepDOC") == "Plain Text"
    )
    doc = {
        "docnm_kwd": filename
    }
    title = re.sub(r"\.[a-zA-Z]+$", "", doc["docnm_kwd"])
    if plain_pdf_mode:
        doc["title_tks"] = _simple_tokenize(title)
        doc["title_sm_tks"] = _simple_tokenize(title, include_unigrams=True)
    else:
    ...
    if re.search(r"\.pdf$", filename, re.IGNORECASE):
        plain_text_mode = plain_pdf_mode
        if plain_text_mode:
        ...
        if plain_text_mode:
            ...
            res.extend(_fast_tokenize_chunks(chunks, doc))
            return res
