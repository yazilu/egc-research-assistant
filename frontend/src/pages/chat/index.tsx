...
export default function Index() {
  ...
  const sendChat = useCallback(
    async (target: API.ChatItem, message: string, attachments?: string[]) => {
      ...
      try {
        const res = await api.session.chat({
          id: id!,
          message,
          local_search: sessionStore.useDeep || sessionStore.useLocalSearch,
      ...
      function parseData(slice: string) {
        try {
          ...
          if (json?.web_search?.length) {
            target.reference = [
              ...(target.reference || []),
              ...json.web_search.map((s: any) => ({
                document_name: s.title || '网页',
                content_with_weight: s.content || '',
                url: s.url || '',
                _source: 'web_search',
              })),
            ]
          ...
          // 处理 EGC 性能预测结果
          if (json?.prediction_result) {
            target.prediction_result = json.prediction_result
          }
        } catch {
          console.debug('解析失败')
          console.debug(slice)
    ...
    [chat, id, sessionStore.useDeep, sessionStore.useLocalSearch, sessionStore.useWeb],
  )
    ...
    async (message: string, attachments?: string[]) => {
      ...
      if (chat.list.length === 0) {
        ...
        const assistantMsg = {
          ...
          type: ChatType.Document,
          documents: [],
          reference: [] as API.Reference[],
        }
        chat.list.push(assistantMsg)
